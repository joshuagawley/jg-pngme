    ...
    fn try_from(value: [u8; 4]) -> Result<Self, Self::Error> {
        for char in value {
            if !(char.is_ascii_lowercase() || char.is_ascii_uppercase()) {
                return Err(anyhow!("Not valid chunk"));
            }
        }
        Ok(Self { code: value })
    }
...
impl FromStr for ChunkType {
    ...
    fn from_str(s: &str) -> std::result::Result<Self, Self::Err> {
        let result: Result<[u8; 4], TryFromSliceError> = s.as_bytes().try_into();
        return match result {
            Ok(s) => ChunkType::try_from(s),
            Err(e) => Err(Self::Err::new(e))
        }
    }
}
...
impl Display for ChunkType {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        write!(f, "{}", std::str::from_utf8(self.code.as_slice()).unwrap())
    }
}
